            ...
            for r in results:
                if r.bias in ["LONG", "SHORT"] and r.setup_score >= alert_threshold:
                    if can_alert(r.symbol, now, cooldown_minutes):
                        ...
                        # Optional email alert (credentials stored in Streamlit Secrets)
                        if enable_email:
                            try:
                                ...
                                to_addr = email_to or cfg.get("to_email", "")
                                if smtp_user and smtp_password and to_addr:
                                    subject = f"Ztockly scalp alert: {row.get('Symbol')} {row.get('Bias')} (Score {row.get('Score')})"
                                    payload = {
                                        "time": row.get("AsOf"),
                                        ...
                                        "entry": row.get("Entry"),
                                        "stop": row.get("Stop"),
                                        "tp1": row.get("TP1") if "TP1" in row else row.get("Target 1R"),
                                        "tp2": row.get("TP2") if "TP2" in row else row.get("Target 2R"),
                                        "why": row.get("Why"),
                                    ...
                                    st.session_state.email_errors.append("Email enabled but missing secrets (smtp_user/smtp_password/to_email).")
